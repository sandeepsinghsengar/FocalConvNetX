from torch.nn import LayerNorm, Module, Sequential, Linear, Dropout, Conv1d, BatchNorm1d
...
class PreNormResidual(Module):
    ...
    def forward(self, x):
        return self.fn(F.gelu(self.norm(x))) + x
...
class Dropsample(nn.Module):
    ...
    def forward(self, x):
        ...
        keep_mask = torch.empty((x.shape[0], 1, 1, 1), device=device).uniform_() > self.prob
